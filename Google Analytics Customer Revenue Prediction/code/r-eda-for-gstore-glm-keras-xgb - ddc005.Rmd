...
For long time period forecasting, we use auto.arima model. We need to forecast for the period of
`r h` days.
We make use of **mean pageviews** (a regression term):
...
differ notably. Thus, they can cause overfitiing, but **year**, **day** and **hour** can be useful.
...
# Autoencoder machine learning model
...
n_ae <- 4
```
Let’s train a machine learning model, which compresses the input space to `r n_ae` components:
